...
def generate_samples(model_dir, model_type, n_gen=None, years=None, device='cpu'):
    ...
    train_data = train_data_clean[:n_train]  # Use same subset as training
    ...
    if n_gen is not None:
        ...
        output_data = z_gen.reshape(n_gen, -1)  # [n_gen, 396]
        ...
        n_days_per_year = 365  # Assuming non-leap years
        n_days = years * n_days_per_year

        # Generate annual maxima for specified years
        with torch.no_grad():
        ...
        for year in range(years):
            ...
            year_maxima = annual_maxima[year]  # [18, 22]

            for i in range(18):
                for j in range(22):
                    max_value = year_maxima[i, j]
                    if max_value <= 0:  # Handle non-positive maxima
                        daily_rainfall[year_start:year_end, i, j] = 0
                        continue

                    # Use exponential distribution for daily values, scaled to max
                    mean_daily = max_value / 50  # Heuristic: daily mean is 1/50th of max
                    scale = mean_daily
                    daily_values = expon.rvs(scale=scale, size=n_days_per_year - 1)
                    daily_values = np.clip(daily_values, 0, max_value * 0.99)  # Cap below max

                    # Insert the annual maximum at a random day
        ...
        output_data = daily_rainfall.reshape(n_days, -1)  # [n_days, 396]
    ...
    pd.DataFrame(output_data).to_csv(output_file, header=None, index=False)
